        ...
        onInit: function (finder) {
            finder.on('files:choose', function (evt) {
                ...
                files.forEach(function (file) {
                    var _extenstion = file._extenstion;
                    var name = file.attributes.name;
                    var value = _extenstion=='mp4' ? name+'__video__'+file.getUrl() : file.getUrl();
                    var div = _extenstion=='mp4' ? '<div class="file__name">'+name+'</div>' : '';
                    var url = _extenstion=='mp4' ? '/public/backend/plugins/ckfinder/skins/neko/file-icons/128/video.png?ckfver=513698098' : file.getUrl();
                    var result = '<div class="image__thumbnail image__thumbnail--style-1">' +
                        ...
                        '<input type="hidden" name="gallery[]" value="' + value + '">' +div
                        '</div>';
...
function addPdfFile(event, el, url, type, table = null) {
    event.preventDefault();
    var target = $(el).closest('.repeater').find('.pdf__content');
    ...
    CKFinder.modal({
        ...
        onInit: function (finder) {
            finder.on('files:choose', function (evt) {
                ...
                var img = parent.find('img').first();
                var input = parent.find('input').first();
                var inputName = parent.find('.pdf-name-hidden');
                var file = evt.data.files.first();
                var name = file.attributes.name;
                var ex = file._extenstion;
                if(ex=='pdf' || ex=='xlsx' || ex=='docx'){
                    var url = file.getUrl();
                    parent.find('.pdf__name').html(name);
                    parent.find('.pdf__error').html('');
                    input.val(url);
                    inputName.val(name);
                }else{
                    parent.find('.pdf__name').html('');
                    input.val('');
                    inputName.val('');
                    parent.find('.pdf__error').html('Vui lòng chọn file đúng định dạng .pdf');
                }
            });
            finder.on('file:choose:resizedImage', function (evt) {
                var parent = $(el).closest('.image');
...
function urlFileDeletePdf(el) {
   $(el).parents('.pdf-item').remove();
}
